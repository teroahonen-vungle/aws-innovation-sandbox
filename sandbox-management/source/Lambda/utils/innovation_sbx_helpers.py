...
def run_stack(act, template_url, credentials, params, stack_name, tags):
    cft = boto3.client('cloudformation',
                       aws_access_key_id=credentials['AccessKeyId'],
                       aws_secret_access_key=credentials['SecretAccessKey'],
                       aws_session_token=credentials['SessionToken'],
    ...
    try:
        cft.create_stack(
            ...
            Tags=[
                ...
                {
                    'Key': 'Mgmt_Account_Name',
                    'Value': tags['Management_Account_Name']
                },
                {
                    'Key': 'Mgmt_Account_ID',
                    'Value': tags['Management_Account_ID']
                }
            ],
            Parameters=params
        )
