...
def del_sgp(ec2, vpc_id):
    ...
    try:
        ...
        message = {'Lambda':'delete_default_vpcs', 'Message': 'Exception occured while deleting Security Groups in default VPC', 'EXCEPTION:': str(e)}
        logger.exception(message)
        raise
...
def create(event, context):
    ...
    try:
        ...
        management_act = props['Management_Account_ID']

        credentials = assume_role(management_act)
    
        delete_default_VPC(credentials)
    
        responseData = {
            "Management_Account_ID" : management_act
        }
